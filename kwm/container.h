...
/* Create a Split Child Container from a Parent Container
    ...
    Notes:
      - Could replace the KWMScreen.SplitRatio with some kind of KWM preferences reference?
...
/* Create Split Containers for each Child of Parent, based on Parent's Container
    ...
    Mutations:
        *Parent->LeftChild->Container  <- node_container
        *Parent->RightChild->Container <- node container
